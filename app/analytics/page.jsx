...
export default function Analytics(){
 ...
 const funnelMeta=useMemo(()=>{if(!phaseB)return {cap:0,capHit:false,legacy:false,note:'Phase B has not produced a persisted run profile yet.'};const cap=Number(phaseB.shortlist_capacity??phaseB.policy?.ai_max_candidates??0),audited=Number(phaseB.ai_offers_submitted||0),capHit=cap>0&&audited>=cap,hasPainStage=valid(phaseB.pain_matched_candidates);if(hasPainStage)return {cap,capHit,legacy:false,note:`Pain-first V2 considered ${fmt(phaseB.pre_ai_candidates_considered)} deterministic candidates and found ${fmt(phaseB.pain_matched_candidates)} with validated pain RAG before AI. ${capHit?`AI audit then sampled the configured capacity of ${fmt(cap)}.`:`AI audited ${fmt(audited)} shortlisted candidates.`} Validation still requires the independent Skeptic gates.`};return {cap,capHit,legacy:true,note:`Legacy Phase B profile: only ${fmt(audited)} candidates reached AI because ai_max_candidates was ${fmt(cap||audited)}. That number is sampling capacity, not evidence that the other ${fmt(phaseA?.unique_commission_eligible_products)} unique eligible products failed quality review.`}},[phaseA,phaseB]);
 const stages=useMemo(()=>{if(!phaseA)return [];const rows=[['Feed records',phaseA.records_seen],['Commercial eligible',phaseA.commission_eligible_records??phaseA.commission_eligible_offers],['Unique eligible',phaseA.unique_commission_eligible_products]];if(valid(phaseB?.pain_matched_candidates))rows.push(['Pain-matched',phaseB.pain_matched_candidates]);if(phaseB){rows.push([funnelMeta.capHit?`AI audited · cap ${fmt(funnelMeta.cap)}`:'AI audited',phaseB.ai_offers_submitted]);rows.push(['Validated',phaseB.saved])}return rows},[phaseA,phaseB,funnelMeta]);
 ...
 const funnel=useMemo(()=>{const palette=[colors.cyan,'#3b82f6',colors.violet,colors.amber,'#a855f7',colors.emerald];return {backgroundColor:'transparent',tooltip:{trigger:'item',backgroundColor:'#0b1220',borderColor:colors.line,textStyle:{color:colors.text},formatter:p=>`${p.name}: <b>${fmt(p.value)}</b>`},series:[{type:'funnel',left:'3%',top:12,bottom:12,width:'94%',minSize:'1%',maxSize:'100%',sort:'none',gap:5,label:{show:true,position:'inside',color:'#f8fafc',fontSize:11,formatter:p=>`${p.name}\n${fmt(p.value)}`},labelLine:{show:false},itemStyle:{borderColor:'#09101a',borderWidth:3},emphasis:{label:{fontSize:12}},data:stages.map(([name,value],i)=>({name,value:Number(value??0),itemStyle:{color:palette[i]||colors.violet}}))}]};},[stages]);
 ...
 return <main className={styles.page}>
  ...
  <div className={styles.gridPrimary}>
   <Panel eyebrow="INTELLIGENCE FUNNEL" title="Capacity vs quality gates" aside={<a href="/configuration">Tune gates →</a>} className={styles.funnelPanel}>{stages.length?<><EChart option={funnel} height={360} ariaLabel="Product intelligence funnel"/><p className={styles.funnelNote} data-legacy={funnelMeta.legacy}>{funnelMeta.note}</p></>:<Empty title="No persisted Phase A profile">The next Product Intelligence run will populate the real funnel.</Empty>}</Panel>
