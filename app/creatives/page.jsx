...
function CreativePreview({row,variant}){
  const stored=String(variant?.asset_url||'');
  if(stored.startsWith('https://'))return <div className={`${styles.preview} ${variant?.id==='reel_9x16'?styles.vertical:variant?.id==='square_1x1'?styles.square:styles.feed}`}><img src={stored} alt={`${row.product_name||'Product'} ${variant?.id||'creative'}`}/></div>;
...
export default function Creatives(){
  ...
  useEffect(()=>{load()},[]);
  const stats=useMemo(()=>({generated:rows.filter(x=>Object.keys(obj(x.creative_pack)).length).length,assets:rows.reduce((n,x)=>n+arr(obj(x.creative_pack).variants).filter(v=>String(v?.asset_url||'').startsWith('https://')).length,0),ready:rows.filter(x=>x.creative_status==='ready').length,review:rows.filter(x=>x.creative_status==='needs_review').length}),[rows]);
  return <main className={styles.page}>
    <header className={styles.hero}><div><span className={styles.eyebrow}>SOCIALMARKET · TOP-20 CAMPAIGN LAB</span><h1>Creatives για τα προϊόντα που αξίζουν πρώτα</h1><p>Ranking → SEO → Creative Director → Creative Skeptic → durable asset → SocialMarket content → SocialScheduler. Τρία stored variants ανά Top‑20 προϊόν, με πραγματική εικόνα και ακριβές affiliate tracking URL.</p></div><div className={styles.actions}><a href="/forecast-products">Products →</a><button onClick={load}>↻ Refresh</button></div></header>
    {error&&<div className={styles.error}>{error}</div>}
    <section className={styles.kpis}><div><span>Top products</span><b>{rows.length}/20</b></div><div><span>Stored assets</span><b>{stats.assets}/60</b></div><div><span>Audit ready</span><b>{stats.ready}</b></div><div><span>Needs review</span><b>{stats.review}</b></div></section>
    {loading?<div className={styles.loading}>Loading Top‑20 campaign packs…</div>:rows.length===0?<section className={styles.empty}><h2>Το final ranking δεν έχει ολοκληρωθεί ακόμη.</h2><p>Η παραγωγική έκδοση δεν δηλώνει success πριν υπάρχουν τουλάχιστον 100 ranked προϊόντα, 20 creative packs και 60 durable assets.</p></section>:<section className={styles.list}>{rows.map(row=>{const pack=obj(row.creative_pack),audit=obj(row.creative_audit),variants=arr(pack.variants);return <article className={styles.card} key={`${row.run_id}-${row.source_record_hash}`}>
