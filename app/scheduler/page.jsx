...
export default function SocialScheduler(){
  const [campaigns,setCampaigns]=useState([]),[posts,setPosts]=useState([]),[connections,setConnections]=useState([]),[ttConnections,setTTConnections]=useState([]);
  ...
  return <main className={styles.page}>
    ...
    {tab==='accounts'?<section className={styles.accountGrid}>
      <Account platform="facebook" title="Facebook Page" conn={connections.find(x=>x.platform==='facebook')}/>
      <Account platform="instagram" title="Instagram Professional" conn={connections.find(x=>x.platform==='instagram')}/>
      ...
      <article className={`${styles.account} ${styles.wide}`}><h2>Publishing requirements</h2><div className={styles.rules}><span>Facebook / Instagram: Meta app + Page/IG authorization required.</span><span>TikTok: Direct Post app authorization required; unaudited clients are restricted by TikTok.</span><span>TikTok media must not contain QR, baked tracking URLs or promotional watermarks.</span><span>Scheduler never publishes until account connection + approval + media readiness are all green.</span></div></article>
    </section>:null}
...
function Account({platform,title,conn}){return <article className={styles.account}><div className={styles.accountTop}><span className={styles.platformIcon}>{icons[platform]}</span><div><small>{names[platform].toUpperCase()}</small><h2>{conn?.display_name||title}</h2></div><i className={cls(conn?.status||'disconnected')}>{conn?.status||'disconnected'}</i></div><p>{conn?.username?`@${conn.username}`:'Meta connection adapter is ready for OAuth configuration.'}</p><button disabled>Connect {names[platform]}</button><small className={styles.note}>Requires Meta App credentials and Page/Instagram permissions.</small></article>}
