...
export default function SocialScheduler(){
  const [campaigns,setCampaigns]=useState([]),[posts,setPosts]=useState([]),[connections,setConnections]=useState([]),[ttConnections,setTTConnections]=useState([]),[metaHealth,setMetaHealth]=useState(null);
  ...
  async function load(){
    ...
    const base=process.env.NEXT_PUBLIC_SUPABASE_URL;if(base){try{const r=await fetch(`${base}/functions/v1/meta-oauth/health`,{cache:'no-store'});setMetaHealth(await r.json())}catch{setMetaHealth({configured:false})}}
  }
  ...
  return <main className={styles.page}>
    ...
    {tab==='accounts'?<section className={styles.accountGrid}>
      ...
      <article className={`${styles.account} ${styles.wide}`}><h2>Publishing requirements</h2><div className={styles.rules}><span>Meta OAuth: {metaHealth?.configured?'configured':'needs app credentials / graph version'}</span><span>TikTok: Direct Post app authorization required; unaudited clients are restricted by TikTok.</span><span>TikTok media must not contain QR, baked tracking URLs or promotional watermarks.</span><span>Scheduler never publishes until account connection + approval + media readiness are all green.</span></div></article>
    </section>:null}
