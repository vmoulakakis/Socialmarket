...
export default async function sitemap(){
  ...
  try{
    const url=`${APPROVED_SUPABASE_URL}/rest/v1/socialmarket_marketplace200_public_v?select=id,updated_at&order=affinity_score.desc&limit=1000`;
    ...
    for(const row of rows||[]){
      if(!row?.id)continue;
      entries.push({url:`${BASE}/marketplace/${encodeURIComponent(String(row.id))}`,changeFrequency:'weekly',priority:.78,lastModified:row.updated_at?new Date(row.updated_at):new Date()});
    }
  }catch{}
  return entries;
}
