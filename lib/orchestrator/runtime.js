...
async function aiPlan(prompt,intent){
  try{
    const sites=registry.sites.map(s=>({id:s.id,role:s.role,managed:s.managed,trafficEligible:trafficEligible(s),knownIssues:s.knownIssues||[]}));
    const r=await agentCompletion({json:true,temperature:0.1,messages:[
      {role:'system',content:`You are the SocialMarket Autonomous Commerce OS planner. Build a minimal DAG from only these capabilities: ${Object.keys(CAPABILITY_MAP).join(', ')}. Never invent evidence. Prefer reversible actions. High-risk capabilities must remain approval-gated. Never route commerce, growth, traffic, preview, or production deployment work to a site whose trafficEligible field is false. Such a site may only be targeted by site-operations/repair work until it is explicitly re-enabled. Return JSON {steps:[{id,capability,dependsOn,input}],reasoning_summary:string}.`},
...
async function executeCapability(step,ctx){
  ...
  if(step.capability==='site.health'){
    const targets=registry.sites.filter(s=>s.productionUrl).map(s=>({id:s.id,url:s.productionUrl,trafficEligible:trafficEligible(s),knownIssues:s.knownIssues||[]}));
  ...
  if(['market.research','product.rank','affinity.design','content.generate'].includes(step.capability)){
    ...
    const r=await agentCompletion({json:true,temperature:0.2,messages:[
      {role:'system',content:`Execute capability ${step.capability} for SocialMarket. Use only supplied context. Separate evidence, inference and unknowns. Do not invent prices, demand, commission, tracking, merchant identity or deployment state. Only trafficEligible sites may be used as commerce/growth destinations. Return JSON.`},
  ...
  if(['code.change','deploy.preview','deploy.production','site.repair','experiment.run'].includes(step.capability)){
    return {state:'COMPLETE',output:{mode:'delegated-tool-action',capability:step.capability,ready:true,requirements:['Git-backed target or explicit adapter credentials','target must be trafficEligible for commerce/growth deployment','verification before production','rollback path'],note:'Runtime records and routes the action; execution is performed by the connected deployment/code adapter.'}};
  }
