...
async function context(){
  ...
  return {programs,pain_clusters:pains,themes,data_semantics:{merchant_missing_metrics:'null_not_zero',pain_taxonomy:'canonical_validated_only'}}
}
...
Deno.serve(async(req)=>{
  if(req.method==='GET')return json({ok:true,service:'product-intelligence-gateway',version:'1.5',deepseek_configured:Boolean(DEEPSEEK_KEY),deepseek_model:DEEPSEEK_MODEL,db_max_connections_per_instance:1,data_semantics:'missing_not_zero;canonical_pains_only'})
  if(req.method!=='POST')return json({error:'method_not_allowed'},405)
  try{
    await auth(req);const b=await req.json();const action=String(b.action||'')
    if(action==='health')return json({ok:true,deepseek_configured:Boolean(DEEPSEEK_KEY),deepseek_model:DEEPSEEK_MODEL,version:'1.5'})
