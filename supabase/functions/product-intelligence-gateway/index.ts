...
async function context(){
  const programs=await sql`
    with merchant_score as (
      select merchant_id,
             ...
             max(competition_score) as competition_score,
    ...
    select mp.id as merchant_program_id,mp.merchant_id,mp.program_name,mp.raw_commission_pct,mp.raw_flat_commission,
           ...
           ms.solution_whitespace_score,ms.demand_beacon_score,ms.demand_score,ms.competition_score,ms.trust_score,ms.confidence,
           coalesce(pol.promotion_mode,'eligible') as promotion_mode,
  ...
  const pains=await sql`
    select sc.id,sc.cluster_type,sc.canonical_text,sc.category,sc.subcategory,sc.evidence_count,sc.source_diversity,sc.demand_score,
           sc.competition_score,sc.pain_severity,sc.commercial_intent,sc.audit_score,sc.confidence
    from evidence.semantic_clusters sc
    where sc.validation_status='validated'
      and sc.cluster_type in ('pain','complaint','unmet_need','alternative_request')
      and exists(select 1 from catalog.taxonomy_nodes c where c.node_type='category' and c.semantic_status='validated' and c.active and c.name=sc.category)
      and (sc.subcategory is null or exists(select 1 from catalog.taxonomy_nodes s join catalog.taxonomy_nodes c on c.id=s.parent_id where s.node_type='subcategory' and s.semantic_status='validated' and s.active and c.semantic_status='validated' and c.active and c.name=sc.category and s.name=sc.subcategory))
    order by coalesce(sc.commercial_intent,0) desc,coalesce(sc.pain_severity,0) desc,coalesce(sc.demand_score,0) desc
    limit 1200`
  ...
  return {programs,pain_clusters:pains,themes,data_semantics:{merchant_missing_metrics:'null_not_zero',pain_taxonomy:'canonical_validated_only',merchant_dominance:'diversity_signal_not_block'}}
}
...
const ENRICH_SYSTEM=`You are SocialMarket Product Research Agent. Analyze only supplied product facts, merchant context and supplied canonical validated pain/theme RAG. Never invent product features, prices, commission, merchant facts, taxonomy, pain evidence, metrics or IDs. Missing merchant demand/competition/trust is UNKNOWN, not zero. A product is useful only when it plausibly solves one or more supplied validated pain/unmet-need clusters. Category/subcategory must be canonical SocialMarket product taxonomy supported by the product facts and RAG; brand, location, theme, promotion, navigation and service-policy labels are never taxonomy. Produce JSON {"items":[...]}. For every input source_record_hash return: source_record_hash, canonical_title, brand_name, model_name, category, subcategory, human_description (Greek, evidence-grounded, no hype), semantic_text, pain_cluster_ids (ONLY IDs supplied in pain_rag), theme_ids (ONLY IDs supplied in theme_rag), pain_gap_fit_score 0-100, seasonal_theme_score 0-100, product_evidence_confidence 0-100, pain_rationale, theme_rationale, unsupported_claims[]. If evidence is weak, use low scores, empty IDs and category-level/null subcategory rather than inventing precision.`

const AUDIT_SYSTEM=`You are SocialMarket Product Skeptic/Audit Agent. Try to prove Product Research wrong. Use only raw product facts, merchant context, supplied canonical RAG and proposed enrichment. Never invent IDs, taxonomy or facts. Missing merchant metrics are unknown, not zero. Reject products that do not clearly solve a supplied validated pain/unmet need, whose taxonomy is non-canonical or unsupported, whose description claims unsupported features, or whose RAG match is keyword noise. Commission >= EUR10 is an immutable economic floor. Merchant dominance/high concentration is a diversity signal, not a rejection rule; only an explicit evidenced merchant policy block is a hard merchant exclusion. Produce JSON {"items":[...]}. For each source_record_hash return verdict exactly validated|needs_review|rejected, pain_cluster_ids ONLY from supplied pain_rag, theme_ids ONLY from supplied theme_rag, pain_scores object keyed by accepted pain IDs, theme_scores object keyed by accepted theme IDs, pain_gap_fit_score 0-100, seasonal_theme_score 0-100, product_evidence_confidence 0-100, identity_score 0-100, source_quality_score 0-100, source_diversity_score 0-100, contradiction_score 0-100 where 100 means no material contradiction, taxonomy_score 0-100, demand_validation_score 0-100, competition_validation_score 0-100, pain_validation_score 0-100, social_validation_score 0-100, overall_score 0-100, pain_rationale, theme_rationale, audit_summary, reasons[], contradictions[]. Use validated only for genuinely evidence-backed canonical solver products.`
...
async function upsertItem(x:any){
  ...
  const policy=await sql`select promotion_mode,dominant_market,reason,metadata from catalog.merchant_promotion_policy where merchant_id=${merchantId} limit 1`
  if(policy[0]?.promotion_mode==='blocked')throw new Error(`merchant_blocked_by_explicit_policy:${String(policy[0]?.metadata?.block_reason_code||'unknown')}`)
  let pr=await sql`select id from catalog.products where canonical_key=${key} limit 1`
  let productId=pr[0]?.id
  if(!productId){
    pr=await sql`insert into catalog.products(canonical_key,canonical_title,brand_name,model_name,gtin,mpn,category,subcategory,status,semantic_text,metadata) values(${key},${String(x.canonical_title||x.product_name_raw||'Product').slice(0,500)},${x.brand_name||null},${x.model_name||null},${x.gtin||null},${x.mpn||null},${x.category||null},${x.subcategory||null},'validated',${x.semantic_text||null},${sql.json({human_description:x.human_description||null,source:'product-intelligence-v1'})}) returning id`;productId=pr[0].id
  }else{
    await sql`update catalog.products set canonical_title=${String(x.canonical_title||x.product_name_raw||'Product').slice(0,500)},brand_name=coalesce(${x.brand_name||null},brand_name),model_name=coalesce(${x.model_name||null},model_name),gtin=coalesce(${x.gtin||null},gtin),mpn=coalesce(${x.mpn||null},mpn),category=coalesce(${x.category||null},category),subcategory=${x.subcategory||null},status='validated',semantic_text=coalesce(${x.semantic_text||null},semantic_text),metadata=metadata||${sql.json({human_description:x.human_description||null})},updated_at=now() where id=${productId}`
  }
  ...
  const s=x.scores||{}
  const snap=await sql`insert into intel.product_intelligence_snapshots(product_id,offer_id,merchant_id,pain_gap_fit_score,merchant_opportunity_score,greek_demand_score,competition_score,seasonal_theme_score,merchant_trust_score,commission_score,discount_score,product_evidence_confidence,dominant_market_penalty,final_opportunity_score,validation_status,audit_summary,evidence_count,metadata) values(${productId},${offerId},${merchantId},${s.pain_gap_fit_score??null},${s.merchant_opportunity_score??null},${s.greek_demand_score??null},${s.competition_score??null},${s.seasonal_theme_score??null},${s.merchant_trust_score??null},${s.commission_score??null},${s.discount_score??null},${s.product_evidence_confidence??null},0,${s.final_opportunity_score??null},'validated',${x.audit_summary||null},${Number(x.evidence_count||0)},${sql.json({audit:x.audit||{},enrichment:x.enrichment||{},method:'agentic-rag-semantic-v1',merchant_dominance:policy[0]?.dominant_market||false})}) returning id`
  const a=x.audit||{}
  await sql`insert into evidence.audit_results(entity_type,entity_id,target_type,target_id,audit_agent,identity_score,source_quality_score,source_diversity_score,contradiction_score,taxonomy_score,demand_validation_score,competition_validation_score,pain_validation_score,social_validation_score,overall_score,verdict,reasons,contradictions,methodology_version,metadata) values('product',${productId},'product_intelligence_snapshot',${snap[0].id},'product-skeptic-semantic-v1',${a.identity_score??null},${a.source_quality_score??null},${a.source_diversity_score??null},${a.contradiction_score??null},${a.taxonomy_score??null},${a.demand_validation_score??null},${a.competition_validation_score??null},${a.pain_validation_score??null},${a.social_validation_score??null},${a.overall_score??null},'validated',${sql.json(a.reasons||[])},${sql.json(a.contradictions||[])},'product-audit-semantic-v1',${sql.json({source_record_hash:hash})})`
  if(x.semantic_text){
    await sql`insert into evidence.semantic_clusters(entity_type,entity_id,cluster_type,canonical_text,category,subcategory,evidence_count,source_diversity,demand_score,competition_score,pain_severity,commercial_intent,audit_score,confidence,validation_status,embedding_status,metadata,updated_at) values('product',${productId},'product_solution',${String(x.semantic_text).slice(0,5000)},${x.category||null},${x.subcategory||null},${Number(x.evidence_count||0)},${Math.min(10,(x.pain_matches||[]).length)},${s.greek_demand_score??null},${s.competition_score??null},${s.pain_gap_fit_score??null},${s.commission_score??null},${a.overall_score??null},${clamp(Number(s.product_evidence_confidence||0))/100},'validated','pending',${sql.json({offer_id:offerId,expected_commission_eur:x.expected_commission_eur,final_opportunity_score:s.final_opportunity_score,taxonomy_policy:'closed_semantic_product_taxonomy_v1'})},now()) on conflict(entity_type,entity_id,cluster_type,md5(canonical_text)) do update set validation_status='validated',embedding_status=case when evidence.semantic_clusters.validation_status='validated' and evidence.semantic_clusters.embedding_status='ready' then 'ready' else 'pending' end,metadata=evidence.semantic_clusters.metadata||excluded.metadata,updated_at=now()`
  }
  return {product_id:productId,offer_id:offerId,snapshot_id:snap[0].id}
}

Deno.serve(async(req)=>{
  if(req.method==='GET')return json({ok:true,service:'product-intelligence-gateway',version:'1.6',deepseek_configured:Boolean(DEEPSEEK_KEY),deepseek_model:DEEPSEEK_MODEL,db_max_connections_per_instance:1,data_semantics:'missing_not_zero;canonical_pains_only;dominance_diversity_not_block'})
  if(req.method!=='POST')return json({error:'method_not_allowed'},405)
  try{
    await auth(req);const b=await req.json();const action=String(b.action||'')
    if(action==='health')return json({ok:true,deepseek_configured:Boolean(DEEPSEEK_KEY),deepseek_model:DEEPSEEK_MODEL,version:'1.6',merchant_block_policy:'explicit_block_only'})
