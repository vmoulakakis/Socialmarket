-- Keep production publishing claims disabled until migration/import/dry-run are verified.
insert into public.app_settings(key,value)
values ('socialscheduler_control','{"enabled":false,"reason":"awaiting_outbox_cutover_validation"}'::jsonb)
...
begin
  select coalesce((s.value->>'enabled')::boolean,false)
   ...
   where s.key='socialscheduler_control';
