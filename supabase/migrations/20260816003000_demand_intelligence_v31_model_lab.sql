-- Demand Intelligence V3.1 model-lab persistence.
-- Additive only: canonical market metrics remain owned by semantic_category_market_v2.
...
create index if not exists demand_model_lab_runs_taxonomy_generated_idx
  on intel.demand_model_lab_runs(taxonomy_id,generated_at desc);
...
begin
  ...
  select jsonb_build_object(
    ...
    'status',x.status,
    'source_market_observed_at',x.source_market_observed_at,
    'generated_at',x.generated_at,
    'analysis',x.analysis,
    'metadata',x.metadata
  ) into out_json
  from intel.demand_model_lab_runs x
  where x.taxonomy_id=p_taxonomy_id and x.geography='GR'
