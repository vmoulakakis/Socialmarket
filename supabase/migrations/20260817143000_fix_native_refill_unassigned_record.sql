-- Final deployment hardening: avoid reading fields from an unassigned PL/pgSQL
-- RECORD when a native-format slot has no pain-solver selection path.
--
-- Root cause: `c record; c := null; ... if c.id is null` is unsafe when no
-- SELECT INTO has assigned a row structure to `c` (for example an Instagram
-- reel/story slot before ranked-product fallback). PostgreSQL raises 55000:
-- `record "c" is not assigned yet`.
--
-- Keep the candidate record, but track assignment explicitly with a boolean.
...
begin
  ...
  for s in select * from publish.generate_slots_v3(now(),p_hours) loop
    ...
    if v_prefer_site then
      ...
      -- Generic pain-solver content is allowed only in feed/post placements.
