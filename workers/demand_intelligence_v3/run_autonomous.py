...
def oidc(force=False):
    ...
    r.raise_for_status();_token=r.json()['value'];_token_at=time.time();return _token
...
def main():
    items=(call('taxonomy_list').get('items') or [])[:MAX_TAXONOMIES]
    stats={'total':len(items),'completed':0,'withheld':0,'failed':0,'errors':[]}
    for i,item in enumerate(items,1):
        tid=str(item.get('taxonomy_id') or '')
        if not tid: continue
        try:
            ctx=call('context',taxonomy_id=tid).get('context') or {}
            result=analyze(ctx)
            call('save',taxonomy_id=tid,analysis=result)
            decision=str(result.get('temporal_lab',{}).get('decision') or '')
            ...
            stats['failed']+=1;stats['errors'].append({'taxonomy_id':tid,'error':f'{type(exc).__name__}:{exc}'[:500]})
            print(json.dumps(stats['errors'][-1],ensure_ascii=False),flush=True)
