...
def main(pattern='shards/*.jsonl.gz'):
    total=price_150=with_tracking=with_image=eligible_basic=0
    ...
    for path in glob.glob(pattern):
        with gzip.open(path,'rt',encoding='utf-8') as f:
            for line in f:
                ...
                price=x.get('price') or 0;instock=x.get('in_stock');tracking=bool(x.get('tracking_url'));image=bool(x.get('image_url') or x.get('thumb_url'));discount=x.get('discount_pct') or 0
                categories[x.get('category_raw') or 'Uncategorized']+=1
                if price>=150:price_150+=1
                if tracking:with_tracking+=1
                if image:with_image+=1
                if x.get('hard_gate_pass'):eligible_basic+=1
    ...
    result={'total':total,'price_150_plus':price_150,'with_tracking_url':with_tracking,'with_image':with_image,'basic_gate_candidates':eligible_basic,'top_categories':categories.most_common(50),'price_bands':dict(price_bands),'discount_bands':dict(discount_bands)}
