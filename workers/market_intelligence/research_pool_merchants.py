...
def main():
    stats=json.load(open('merchant-stream-stats.json',encoding='utf-8'));cats=json.load(open('category-stream-stats.json',encoding='utf-8'))
    by=defaultdict(list)
    ...
    for i,(m,rows) in enumerate(sorted(by.items(),key=lambda kv:len(kv[1]),reverse=True),1):
        ...
        purchase=clamp(18*math.log1p(avg_times));stability=clamp(35+16*math.log1p(eligible));internal=clamp(purchase*.62+stability*.38)
        domain=None
        for p in rows[:8]:
        ...
        sigs=[];neg_domains=set();credible_domains=set()
        ...
        terms=representative_terms(rows);appear=queries=0
        if domain:
            for term in terms:
        ...
        feed_visibility=clamp(share*1800);visibility=clamp(dominance*62+feed_visibility*.38)
        ...
        result[m]={'merchant':m,'domain':domain,'internal_trust':round(internal,2),'external_reputation':round(external,2),'external_confidence':round(evidence_conf,4),'trust_score':round(trust,2),'feed_share':round(share,6),'serp_dominance':round(dominance,4),'visibility_penalty':round(visibility,2),'merchant_suitability':round(suitability,2),'risk_flag':risk_flag,'mega_visibility_kill':mega_kill,'domain_age_years':round(age,2) if age is not None else None,'negative_credible_domains':sorted(neg_domains),'evidence_count':len(review_results),'representative_terms':terms}
        if i%20==0:print(json.dumps({'researched':i,'merchants':len(by)}),flush=True)
    with open('merchant-pool-research.json','w',encoding='utf-8') as f:json.dump(result,f,ensure_ascii=False,indent=2)
    print(json.dumps({'merchants':len(result),'risk_flags':sum(x['risk_flag'] for x in result.values()),'mega_visibility_kills':sum(x['mega_visibility_kill'] for x in result.values())}))
