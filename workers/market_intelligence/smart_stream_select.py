...
def compact(p,pre):
    ...
    q['extra_images']=(q.get('extra_images') or [])[:4]
    q['_pre_score']=round(pre,3)
    return q
...
def main(path):
    ...
    for raw in iter_records(path):
        ...
        if not p.get('hard_gate_pass'):
            for r in (p.get('eligibility_reason') or {}).get('reasons',[]): reasons[r]+=1
    ...
    # Give sparse categories a small discovery bonus, then bound the local shortlist.
    ranked=[]
