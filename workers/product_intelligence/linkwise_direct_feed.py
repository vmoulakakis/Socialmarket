...
The Google Drive 3.84 GB JSON is a cache/snapshot. Production should prefer the
Linkwise joined-program feed directly and keep Drive as a fallback snapshot only.
"""
...
# Do not request fields that the production worker does not use. Deeplink field
# is resolved by the probe and then mapped to tracking_url during normalization.
CORE_FIELDS=(
...
def _session():
    s=requests.Session()
    s.headers.update({'User-Agent':'SocialMarketAI/3.6 direct-feed','Accept-Encoding':'gzip, deflate'})
    return s


def probe_field(field,session=None):
    s=session or _session();url=feed_url(('product_id',field,'price'))
    try:
        with s.get(url,stream=True,timeout=(15,30),allow_redirects=True) as r:
            result={'field':field,'status':r.status_code,'content_type':r.headers.get('content-type'),'content_length':r.headers.get('content-length')}
            if r.status_code>=400:
                result['error']=r.text[:240]
                return result
            buf=b''
            for chunk in r.iter_content(32768):
                if chunk:
                    buf+=chunk
                    if len(buf)>=196608:break
            text=buf.decode('utf-8','replace')
            keys=sorted(set(re.findall(r'"([A-Za-z0-9_]+)"\s*:',text)))
            result['keys']=keys[:80]
            result['field_present']=field in keys
            result['linkwise_tracking_shape']=bool(re.search(r'https?[^"\\]*(?:go|affiliate)\.linkwi\.se',text,re.I))
            result['bytes_sampled']=len(buf)
            return result
    except Exception as exc:
        return {'field':field,'status':0,'error':f'{type(exc).__name__}: {exc}'}
...
def resolve_deeplink_field():
    ...
    # Some providers return a syntactically accepted field before a first URL is
    # encountered. Prefer a present 200 field, but never silently invent a name.
...
def direct_columns():
    explicit=os.getenv('LINKWISE_DEEPLINK_FIELD','').strip()
    field=explicit or resolve_deeplink_field()[0]
    return (*CORE_FIELDS,field),field
...
def download(output,minimum_bytes=10_000_000,max_attempts=3):
    ...
    for attempt in range(1,max_attempts+1):
        try:
            ...
            with _session().get(url,stream=True,timeout=(30,900),allow_redirects=True) as r:
                ...
                with tmp.open('wb') as f:
                    for chunk in r.iter_content(1024*1024):
                        ...
                        if written and written%(250*1024*1024)<1024*1024:
                            print(json.dumps({'direct_linkwise_download_bytes':written,'attempt':attempt}),flush=True)
            ...
            last=exc
            print(json.dumps({'warning':'direct_linkwise_download_attempt_failed','attempt':attempt,'error':str(exc)[:600]}),flush=True)
...
def main():
    ...
    if args.probe:
        field,results=resolve_deeplink_field();print(json.dumps({'ok':True,'resolved_deeplink_field':field,'results':results},ensure_ascii=False));return 0
