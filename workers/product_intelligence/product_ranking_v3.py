...
def rank_gateway(action, **payload):
    ...
    try:
        ...
        raise RuntimeError(f'ranking gateway {action} failed: {exc.code} {msg[:1000]}')
...
def deterministic_metrics(item):
    raw,merchant=item.get('_raw') or {},item.get('merchant') or {}
    ...
    commission=commission_score(raw.get('expected_commission_eur')); discount=discount_score(raw.get('discount_pct')); purchase=purchase_signal(raw.get('times_bought')); pain=pain_signal(item); seasonal=seasonal_signal(item)
    base=demand*.22+whitespace*.18+commission*.17+inverse*.12+trust*.10+discount*.07+purchase*.06+pain*.04+seasonal*.04
    return {'merchant_demand_score':round(demand,3),'competition_score':None if competition is None else round(clamp(num(competition)),3),'inverse_competition_score':round(inverse,3),'merchant_whitespace_score':round(whitespace,3),'merchant_trust_score':round(trust,3),'commercial_score':round(commission,3),'discount_score':round(discount,3),'purchase_signal_score':round(purchase,3),'pain_signal_score':round(pain,3),'seasonal_score':round(seasonal,3),'deterministic_rank_score':round(clamp(base),3)}


def preselect(products,context):
    heap=[];seq=0;considered=0
    for product in products:
        considered+=1;item=v1.build_ai_item(product,context);item['_rank_metrics']=deterministic_metrics(item);seq+=1;entry=(item['_rank_metrics']['deterministic_rank_score'],seq,item)
    ...
    if len(selected)<AI_MAX:
        ...
        for _,_,item in ordered:
            ...
            if len(selected)>=AI_MAX: break
    return selected,{'eligible_candidates_considered':considered,'preselected':len(ordered),'ai_shortlist':len(selected),'unique_merchants':len({str((x.get('merchant') or {}).get('merchant_id')) for x in selected}),'unique_categories':len({str((x.get('_raw') or {}).get('category_raw')) for x in selected})}


def wire_item(item):
    return {'product':item['product'],'merchant':item['merchant'],'pain_rag':item.get('pain_rag') or [],'theme_rag':item.get('theme_rag') or [],'deterministic_metrics':item['_rank_metrics'],'ranking_semantics':{'objective':'promotion opportunity in Greece','pain_missing_is_not_rejection':True,'missing_competition_gets_no_inverse_bonus':True}}
...
def rank_with_ai(items):
    ...
    for start in range(0,len(items),AI_BATCH):
        ...
        try:
            ranked=rank_gateway('rank',items=wire,thinking='auto').get('items',[]);by_hash={str(x.get('source_record_hash')):x for x in ranked}
            ...
            audited=rank_gateway('rank_audit',items=audit_wire,thinking='auto').get('items',[]) if audit_wire else [];audit_by={str(x.get('source_record_hash')):x for x in audited}
...
def final_row(item,ai):
    raw,merchant,m=item['_raw'],item['merchant'],item['_rank_metrics'];ranking,audit=ai.get('ranking') or {},ai.get('audit') or {}
    ...
    raw_score=m['merchant_demand_score']*.20+m['merchant_whitespace_score']*.15+m['commercial_score']*.12+m['inverse_competition_score']*.10+m['merchant_trust_score']*.08+m['discount_score']*.07+m['purchase_signal_score']*.05+m['pain_signal_score']*.05+m['seasonal_score']*.05+product_fit*.05+creative*.04+value*.02+confidence*.02
    score=clamp(raw_score*(.85+.15*(100-risk)/100));band='PROMOTE_NOW' if score>=82 else 'HIGH_POTENTIAL' if score>=72 else 'TEST' if score>=62 else 'WATCHLIST'
    channels=ranking.get('recommended_channels') or [];channels=channels if isinstance(channels,list) else [str(channels)]
    risk_flags=audit.get('risk_flags') or ranking.get('risk_flags') or [];risk_flags=risk_flags if isinstance(risk_flags,list) else [str(risk_flags)]
    return {'source_record_hash':raw['source_record_hash'],'canonical_key':raw['canonical_key'],'external_product_id':raw.get('external_product_id'),'merchant_id':merchant['merchant_id'],'merchant_program_id':merchant.get('merchant_program_id'),'merchant_name':raw.get('merchant_name') or merchant.get('canonical_name'),'product_name':raw.get('product_name') or item['product'].get('product_name') or 'Product','brand_name':raw.get('brand_name'),'model_name':raw.get('model_name'),'category':ranking.get('category') or raw.get('category_raw'),'subcategory':ranking.get('subcategory'),'effective_price':raw.get('price'),'full_price':raw.get('full_price'),'discount_pct':raw.get('discount_pct'),'expected_commission_eur':raw.get('expected_commission_eur'),'tracking_url':raw.get('tracking_url'),'image_url':raw.get('image_url') or raw.get('thumb_url'),'in_stock':raw.get('in_stock'),'times_bought':raw.get('times_bought'),'merchant_demand_score':m['merchant_demand_score'],'competition_score':m['competition_score'],'merchant_whitespace_score':m['merchant_whitespace_score'],'merchant_trust_score':m['merchant_trust_score'],'pain_signal_score':m['pain_signal_score'],'seasonal_score':m['seasonal_score'],'commercial_score':m['commercial_score'],'purchase_signal_score':m['purchase_signal_score'],'ai_product_fit_score':product_fit,'ai_creative_score':creative,'ai_value_score':value,'ai_confidence':confidence,'ai_risk_score':risk,'rank_score':round(score,3),'rank_band':band,'promotion_angle':ranking.get('promotion_angle'),'promotion_reason':ranking.get('promotion_reason') or ranking.get('rationale'),'audience':ranking.get('audience'),'recommended_channels':channels,'risk_flags':risk_flags,'evidence_summary':{'pain_matches':len(item.get('_pains') or []),'theme_matches':len(item.get('_themes') or []),'deterministic_rank_score':m['deterministic_rank_score'],'audit_reasons':audit.get('reasons') or []},'ai_summary':audit.get('audit_summary') or ranking.get('rationale')}
...
def main(feed):
    ...
    context=v1.gateway('context');reused=False
    ...
    shortlist,shortlist_stats=preselect(v1.iter_best_offers(db,v1.AI_OFFERS_PER_PRODUCT),context);ai_outputs,ai_stats=rank_with_ai(shortlist)
    rows=[final_row(item,ai_outputs[str(item['product']['source_record_hash'])]) for item in shortlist if str(item['product']['source_record_hash']) in ai_outputs];rows.sort(key=lambda x:(x['rank_score'],x['ai_confidence'],num(x['expected_commission_eur'])),reverse=True);rows=rows[:SAVE_LIMIT]
    run_key=f"{os.getenv('GITHUB_RUN_ID') or datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%SZ')}-{os.getenv('GITHUB_RUN_ATTEMPT','1')}";start=rank_gateway('ranking_start',run_key=run_key,engine_version=ENGINE_VERSION,metadata={'runtime_config_version':cfg.get('_version'),'pain_clusters_available':len(context.get('pain_clusters') or []),'themes_available':len(context.get('themes') or []),'policy':'ranking-first; pain optional'});run_id=start['run_id'];saved=0
    for i in range(0,len(rows),40): saved+=int(rank_gateway('save_rankings',run_id=run_id,items=rows[i:i+40]).get('saved') or 0)
    rank_gateway('ranking_complete',run_id=run_id,records_seen=int(stream_stats.get('records_seen') or 0),eligible_candidates=int(stream_stats.get('commission_eligible_records') or 0),ai_ranked=int(ai_stats.get('ai_ranked') or 0),saved_count=saved,metadata={'shortlist':shortlist_stats,'ai':ai_stats})
    bands=collections.Counter(x['rank_band'] for x in rows);profile={'engine_version':ENGINE_VERSION,'run_key':run_key,**stream_stats,**shortlist_stats,**ai_stats,'saved_rankings':saved,'bands':dict(bands),'top_20':[{k:x.get(k) for k in ('product_name','merchant_name','rank_score','rank_band','expected_commission_eur','promotion_angle','recommended_channels')} for x in rows[:20]],'ranking_policy':'Every deterministic eligible product can compete. Missing pain lowers support but never automatically excludes.'}
