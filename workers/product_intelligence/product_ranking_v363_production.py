...
def _profile(rows,run_key,stream_stats,shortlist_stats,ai_stats,content_stats,saved):
    ...
    return {
        ...
        'ranking_policy':'All eligible products compete deterministically; only a bounded frontier reaches local open-weight AI. Missing pain/forecast gives no bonus but is not fabricated. Independent local skeptic gates the final portfolio.',
    }
...
def _assert_local_health():
    ...
    # Model availability is verified locally. Paid-provider configuration is ignored.
...
def main(feed):
    ...
    if float(v1.MIN_COMMISSION)<15.0:
        raise SystemExit(f'First authoritative Autopilot run requires owner commission floor >= €15; got €{v1.MIN_COMMISSION:.2f}')
    ...
    if not run_id:
        run=production_gateway('ranking_start',run_key=run_key,engine_version=ENGINE_VERSION,metadata={
            'runtime_config_version':cfg.get('_version'),'orchestrator':'product_ranking_v4_local_autopilot','stage':'starting',
            'model_route':'provider_neutral_local_router','local_model':local_ai.LOCAL_MODEL,'paid_llm_required':False,'bulk_feed_to_llm':False,
            'owner_min_expected_commission_eur':float(v1.MIN_COMMISSION),
            'policy':'deterministic bulk filters -> bounded RAG frontier -> local rank -> independent local skeptic -> deterministic SEO -> local Top20 creative + skeptic',
        })
    ...
    try:
        ...
        if v1.REUSE_STAGE and Path(v1.STAGE_DB).exists():
            try:
                db=v1.init_stage(v1.STAGE_DB,reset=False);staged=db.execute('select count(*) from candidates').fetchone()[0];reused=staged>0
        ...
        stage='local_ai_ranking'
        ai_outputs,ai_stats=local_ai.rank_with_local_ai(shortlist)
        ...
        if any(v3.num(x.get('expected_commission_eur'))+1e-9<float(v1.MIN_COMMISSION) for x in rows):
            raise RuntimeError('owner commission hard gate violation detected after local ranking')

        stage='deterministic_seo_local_top20_creatives'
        rows,content_stats=local_ai.enrich_final_rows_local(rows)
        ...
        stage='finalize'
        production_gateway('ranking_complete',run_id=run_id,records_seen=int(stream_stats.get('records_seen') or 0),eligible_candidates=int(stream_stats.get('commission_eligible_records') or 0),ai_ranked=int(ai_stats.get('ai_ranked') or 0),saved_count=saved,metadata={
        ...
        print(json.dumps({'product_ranking_v4_local_autopilot':profile},ensure_ascii=False,default=str),flush=True)
        return profile
