...
Guarantees:
- DB/context/persistence stays in marketplace200-agent-gateway.
- AI planning/research/skeptic reasoning uses the lightweight marketplace200-ai-gateway.
- AliExpress uses the secure compatibility gateway backed by the proven travelai
  server-side credential runtime. No secrets are copied into this workflow.
- Missing/invalid affiliate promotion links fail closed.
"""
...
def _split_gateway(action: str, **payload: Any) -> dict[str, Any]:
    if action=='plan':
        ...
        ai_payload={
            'market':'GR',
            'pain_clusters':list(ctx.get('pains') or [])[:180],
            'market_context':list(ctx.get('markets') or [])[:90],
            'eligible_merchants':[
                ...
                for x in list(ctx.get('programs') or [])[:120]
            ],
            'feedback':list(ctx.get('feedback') or [])[:50],
            'policy':ctx.get('config') or {},
        }
        r=_ai('plan',ai_payload)
        return {'ok':True,'plan':r.get('plan') or {}}
    if action=='evaluate':
        ...
        for raw in items:
            ...
            merged.append({
                **raw,**r,'greek_availability':availability,'product_quality_score':quality,'skeptic_verdict':verdict,
                'quality_decision':'SELECTED' if selected else ('REJECTED' if verdict=='rejected' else 'HOLD'),
                'skeptic_reasons':list(s.get('reasons') or []),'skeptic_blockers':list(s.get('blockers') or []),
                'required_rechecks':list(s.get('required_rechecks') or []),'contradiction_score':_clamp(s.get('contradiction_score')),
            })
...
def _find_tracking_url(value: Any) -> str:
    if isinstance(value,str):
        return value.replace('http://','https://',1) if value.startswith(('https://s.click.aliexpress.com/','http://s.click.aliexpress.com/')) else ''
...
def discover_aliexpress_safe(clusters, excluded):
    ...
    for key,rows in list(buckets.items()):
        ...
        for row in rows:
            ...
            if not _find_tracking_url(row.get('tracking_url')):
                dropped+=1;continue
