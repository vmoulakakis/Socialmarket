...
def ensure_month_plan(db:SupabaseREST):
    ...
    now=datetime.now(timezone.utc);coverage=now+timedelta(days=max(10,horizon//2));cooldown=now-timedelta(hours=6)
    ...
    recent=db.get('product_to_post_runs',f"mode=eq.auto&created_at=gte.{cooldown.isoformat()}&select=id,status,created_at&order=created_at.desc&limit=1") or []
    if existing:return {'created':False,'reason':'forward_coverage_exists'}
    if active:return {'created':False,'reason':'active_auto_run_exists'}
    if recent:return {'created':False,'reason':'six_hour_retry_cooldown','recent_run':recent[0].get('id')}
