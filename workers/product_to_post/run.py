...
MIN_PRICE_EUR = float(os.getenv("P2P_MIN_PRICE_EUR", "100"))
MIN_HIGO = float(os.getenv("P2P_MIN_HIGO", "70"))
MIN_CONFIDENCE = float(os.getenv("P2P_MIN_CONFIDENCE", "0.55"))
...
def eligible(product: dict[str, Any] | None) -> bool:
    ...
    if product.get("market_eligible") is False or product.get("is_preferred_offer") is False:
        return False
    try:
        if float(product.get("price") or 0) < MIN_PRICE_EUR:
            return False
...
def conversion_priority(opp: dict[str, Any], product: dict[str, Any]) -> tuple[float, dict[str, float]]:
    higo = float(opp.get("higo_adjusted") or 0)
    demand = float(opp.get("demand_score") or 0)
    forecast = float(opp.get("forecast_momentum_score") or 0)
    offer = float(opp.get("offer_score") or 0)
    trust = float(product.get("merchant_trust_score") or opp.get("merchant_reliability_score") or 0)
    seller_comp = float(opp.get("seller_competition_score") or 0)
    ad_pressure = float(opp.get("ad_pressure_score") or 0)
    low_comp = max(0.0, min(100.0, 100.0 - (seller_comp * 0.70 + ad_pressure * 0.30)))
    score = (
        0.30 * higo
        + 0.18 * demand
        + 0.12 * forecast
    ...
    return score, {
        ...
        "forecast": forecast,
        "offer": offer,
        "merchant_trust": trust,
        "seller_competition": seller_comp,
...
def select_products(db: SupabaseREST, run: dict[str, Any]) -> list[tuple[dict[str, Any], dict[str, Any] | None, dict[str, Any]]]:
    if run.get("mode") == "manual":
        ...
        if not eligible(product):
            raise RuntimeError("manual_product_failed_price_gate_or_missing_tracking_media")
        opp = opportunity_for(db, pid)
        priority, factors = conversion_priority(opp or {}, product)
        reason = {"mode": "manual", "conversion_priority": round(priority, 2), **factors, "confidence": (opp or {}).get("confidence")}
        return [(product, opp, reason)]

    requested = max(1, min(int(run.get("requested_count") or 1), 30))
    limit = max(40, min(requested * 12, 400))
    rows = db.get(
        "opportunity_scores",
    ...
    for opp in rows:
        ...
        if not eligible(product) or opp.get("competition_kill") is True:
            continue
        pid = product["id"]
        if pid in seen:
            continue
        ...
        higo = float(opp.get("higo_adjusted") or 0)
        if confidence < MIN_CONFIDENCE or higo < MIN_HIGO:
            continue
        priority, factors = conversion_priority(opp, product)
        seen.add(pid)
        reason = {
            "mode": "auto",
            "strategy": "maximum_expected_conversion",
            "conversion_priority": round(priority, 2),
            "confidence": confidence,
            **factors,
        }
        candidates.append((priority, product, opp, reason))

    candidates.sort(key=lambda x: (x[0], float(x[2].get("confidence") or 0), float(x[2].get("higo_adjusted") or 0)), reverse=True)
    selected = [(product, opp, reason) for _, product, opp, reason in candidates[:requested]]
    if not selected:
        raise RuntimeError("auto_selection_returned_no_eligible_products_over_100")
    return selected
...
def upsert_item(db: SupabaseREST, run_id: str, product: dict[str, Any], rank: int, reason: dict[str, Any]) -> dict[str, Any]:
    rows = db.post("product_to_post_items?on_conflict=run_id,product_id", {
        "run_id": run_id, "product_id": product["id"], "selection_rank": rank,
        "selection_reason": reason, "status": "selected", "updated_at": now_iso()
    }, upsert=True)
...
def persist_evidence(db: SupabaseREST, item_id: str, evidence: dict[str, Any]) -> dict[str, Any]:
    rows = db.post("product_enrichment_evidence", {
        ...
        "http_status": evidence.get("http_status"), "facts": evidence.get("facts") or {},
        "source_meta": evidence.get("source_meta") or {}, "content_hash": evidence.get("content_hash"),
    })
...
def render_platform_assets(db: SupabaseREST, run: dict[str, Any], item: dict[str, Any], product: dict[str, Any], opp: dict[str, Any] | None, variants: list[dict[str, Any]]) -> dict[str, str]:
    ...
    for platform in run.get("platforms") or []:
        ...
        job = first_row(db.post("creative_jobs", {
            ...
            "concept_type": "product-to-post-v2", "platform_target": platform, "tracking_url": product["tracking_url"],
        ...
        try:
            ...
            asset = first_row(db.post("creative_assets", {
                ...
                "visual_audit": {"status": "needs_review", "renderer": "product-to-post-v2", "product_fidelity": "source-image-preserved", "qr_checked": platform != "tiktok"},
            }))
            if asset:
...
def process_item(db: SupabaseREST, brain: MarketingBrain, run: dict[str, Any], product: dict[str, Any], opp: dict[str, Any] | None, reason: dict[str, Any], rank: int) -> dict[str, Any]:
    ...
    if status and status >= 400:
        raise RuntimeError(f"affiliate_landing_http_{status}")
    ...
    return {"product_id": product["id"], "product_name": product.get("product_name"), "price": product.get("price"), "conversion_priority": reason.get("conversion_priority"), "angles": len(angles), "variants": len(variants), "assets": len(assets), "calendar_slots": len(slots), "model_routes": [angle_route, *route_log]}
...
def process_run(db: SupabaseREST, run: dict[str, Any]) -> dict[str, Any]:
    ...
    db.patch("product_to_post_runs", f"id=eq.{run['id']}", {
        "status": "needs_approval", "summary": summary, "model_route": "github_models_free_or_deterministic",
        "finished_at": now_iso(), "updated_at": now_iso(), "error": None,
    })
    return summary
